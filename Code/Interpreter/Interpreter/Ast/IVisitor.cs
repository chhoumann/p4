...
namespace Interpreter.Ast
{
    public interface IVisitor
    {
        ...
        void Visit(GameObjectType gameObjectType);
        void Visit(GameObjectContents gameObjectContents);
    }
/*
...
These methods must simply redirect the call to a visiting method on the incoming visitor object which matches the 
...
5. For each behavior that can’t be implemented inside the element hierarchy, 
...
In this case, you can either make these fields or methods public, violating the element’s encapsulation, or nest the visitor class in the element class. The latter is only possible if you’re lucky to work with a programming language that supports nested classes.
